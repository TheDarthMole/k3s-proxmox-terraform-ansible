...
resource "local_file" "k8s_file" {
  content  = templatefile("${path.module}/templates/k8s.tpl", {
    k3s_master_ip = join("\n", [for instance in proxmox_vm_qemu.proxmox_vm_master : join(" ", [instance.default_ipv4_address, var.pvt_key == "" ? "" : "ansible_ssh_private_key_file=${var.pvt_key}"])])
    k3s_node_ip   = join("\n", [for instance in proxmox_vm_qemu.proxmox_vm_workers : join(" ", [instance.default_ipv4_address, var.pvt_key == "" ? "" : "ansible_ssh_private_key_file=${var.pvt_key}"])])
  })
